...
def trainNet(Required_output):
	...
	while Running:
		...
		activation_H = []	
		for j in xrange(len(hiddenL)):
			aa = activation(net_input_H[j],"H")
		#	if aa > 0.99:
		#		aa = 1
		#	else:
		#		aa = 0
			activation_H.append(aa)
		#total_sum = sum(activation_H)
		#kkk1 = []
		#for part in activation_H:
		#	kkk1.append(part/total_sum)
		...
		activation_O = []	
		for j in xrange(len(outL)):
			activation_O.append(activation(net_input_O[j],"O"))
		total_sum = sum(activation_O)
		kkk2 = []
		for part in activation_O:
			kkk2.append(part/total_sum)
		print kkk2

...
def setup(grid):
	...
	for i in range(9):
		offset += 1
		for j in range(9):
			grid[i][j][0] = problem2[offset*9+j]
			if grid[i][j][0] != ".":
				for m in range(1,10):
					try:
...
def solve(mylist,p,d):
	...
	print "Where are we p =",p
	
	(x1,y1) = (mylist[p+1][0],mylist[p+1][1])
	
	print "next x1 y1 grid",x1,y1,grid[x1][y1]
	filename = raw_input()
	...
	if (x1 == x) or (y1 == y):
		...
		for r in range(1,len(grid[x1][y1])):
			...
			if True:
				...
				for kk in range(9):
					...
					if grid[kk][y1][0] == grid[x1][y1][r]:
					
						clash = 1
						print "clash x",grid[x1][y1][r],kk
					if grid[x1][kk][0] == grid[x1][y1][r]:
					
						clash = 1
						print "clash y",grid[x1][y1][r],kk		
					
				if not(clash):
					print "Call solve",p+1,r
					solve(mylist,p+1,r)
					print "p val",p,xcount
					if p == xcount:
						dontloop = True
